<template>
	<AppLayout>
		...
		<PopUp v-model="soldout" width="600px">
			<SoldOut :product="product" :size="size" :qty="getSizeQty" v-model="soldout"></SoldOut>
		</PopUp>
		<PopUp v-model="done">
						...
						<div class="d-flex align-items-baseline flex-column flex-sm-row flex-wrap mb-4">
							<div class="d-flex align-items-baseline flex-wrap">
								...
								<button type="button" class="btn text-dark" :class="[s.size.id === size ? 'btn-primary' : 'btn-link']" v-for="s in product.sizes" :key="Math.random()" v-if="s.size && s.qty !== null" @click="size = s.size.id">
									<del class="text-muted" v-if="s.qty < 1">{{ s.size.name }}</del>
			...
			<div class="container-fluid px-3 px-sm-5">
				<div class="d-flex py-4">
					<div class="me-auto">
						...
						<l :href="route('shop', {categories: [c.category_id]})" class="underline me-2" v-for="c in product.categories" :key="Math.random()" v-if="c.category">
							{{ c.category.name }}
						</l>
					</div>
					<div class="text-end">
